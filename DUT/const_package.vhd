...
-- IDECODE constants
	...
	constant UTYPE_OPCODE_COMMON_PATTERN : STD_LOGIC_VECTOR(6 DOWNTO 0) :=
		"0010111" and "0110111";	--Upper immediate
...
-- Immediate arithmetic, logical, comparison, and shift instructions.
-- andi
...
-- Upper-immediate forms use only the seven-bit opcode for identification.
-- lui
...
-- Register-to-register integer ALU operations.
-- add
...
-- Unconditional jumps and project-specific RETI.
-- jal
...
-- jalr
 ...
 constant INST_RETI                     : STD_LOGIC_VECTOR(31 DOWNTO 0) := 32x"00020067";

-- Conditional branches share an opcode and are separated by funct3.
-- beq
...
-- Signed and unsigned loads of the supported operand widths.
-- lb
...
-- Store width is selected by funct3; address and source fields remain masked.
-- sb
...
-- Privileged/system instructions with fixed encodings.
-- ecall
...
-- eret
	 -- Standard user-mode return encoding. Project interrupt return uses INST_RETI.
	 constant INST_URET 					:	STD_LOGIC_VECTOR(31 DOWNTO 0) := 32x"200073";
	 constant INST_URET_MASK 			:	STD_LOGIC_VECTOR(31 DOWNTO 0) := 32x"cfffffff";

-- CSR register and immediate forms. funct3 selects operation and source.
-- csrrw
...
-- M-extension instruction encodings.
-- mul
...
-- Low-power wait and memory-ordering operations retained for decoder coverage.
-- wfi
...
-- sfence
	 constant INST_SFENCE_VMA 			:	STD_LOGIC_VECTOR(31 DOWNTO 0) := 32x"12000073";
	 constant INST_SFENCE_VMA_MASK 	:	STD_LOGIC_VECTOR(31 DOWNTO 0) := 32x"fe007fff";

-- fence.i
	 constant INST_FENCE_I 				:	STD_LOGIC_VECTOR(31 DOWNTO 0) := 32x"100f";
	 constant INST_FENCE_I_MASK 		:	STD_LOGIC_VECTOR(31 DOWNTO 0) := 32x"707f";

-- End of the decoder's instruction table.
end const_package;
